...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zTrustron spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zTrustron serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("TrustronMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("TrustronMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("TrustronMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("trustron-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        if (fProofOfStake) {
            ...
            while (chainActive.Tip()->nTime < 1471482000 || vNodes.empty() || pwallet->IsLocked() || !fMintableCoins || nReserveBalance >= pwallet->GetBalance() || !masternodeSync.IsSynced() || pwallet->GetBalance() < 10000 * COIN) {
			...
			if(fProofOfStake && pwallet->GetBalance() < 10000 * COIN) {
        ...
        LogPrintf("Running TrustronMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
