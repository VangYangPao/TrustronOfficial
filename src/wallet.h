...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TSN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTSN send
enum ZerocoinSpendStatus {
    ZTSN_SPEND_OKAY = 0,                            // No error
    ZTSN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZTSN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZTSN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZTSN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZTSN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZTSN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZTSN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZTSN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZTSN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZTSN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZTSN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZTSN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZTSN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZTSN_SPENT_USED_ZTSN = 14                       // Coin has already been spend
};
...
public:
    bool MintableCoins(int nHeight);
    bool SelectStakeCoins(std::set<std::pair<const CWalletTx*, unsigned int> >& setCoins, CAmount nTargetAmount, int nHeight) const;
    ...
    */
    ...
    void setZTrustronAutoBackups(bool fEnabled)
    {
    ...
    bool CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime, int nPrevHeight);
    bool MultiSend();
