...
#include "ui_ztrustroncontroldialog.h"
...
std::list<std::string> ZTrustronControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTrustronControlDialog::listMints;

ZTrustronControlDialog::ZTrustronControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTrustronControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTrustronControlDialog::~ZTrustronControlDialog()
{
    delete ui;
}

void ZTrustronControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTrustronControlDialog::updateList()
{
...
void ZTrustronControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTrustronControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTrustron_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTrustronControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTrustronControlDialog::GetSelectedMints()
{
...
void ZTrustronControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
