...
#include "ztrustroncontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTSN ought to be enough for anybody." - Bill Gates, 2017
    ui->zTSNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTSNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTSN->setEnabled(false);
        ui->pushButtonMintzTSN->setToolTip(tr("zTSN is currently disabled due to maintenance."));

        ui->pushButtonSpendzTSN->setEnabled(false);
        ui->pushButtonSpendzTSN->setToolTip(tr("zTSN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTSNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTSN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTSN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTSN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTSN in ") +
...
void PrivacyDialog::on_pushButtonSpendzTSN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTSN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTSN
        sendzTSN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTSN
    sendzTSN();
}

void PrivacyDialog::on_pushButtonZTrustronControl_clicked()
{
    ZTrustronControlDialog* zTrustronControl = new ZTrustronControlDialog(this);
    zTrustronControl->setModel(walletModel);
    zTrustronControl->exec();
}

void PrivacyDialog::setZTrustronControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTrustronSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTSN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Trustron Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTSNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTSN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TSN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTSN</b>";
    ...
    // use mints from zTrustron selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZTrustronControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZTrustronControlDialog::GetSelectedMints();
    }
    ...
    // Clear ztrustron selector in case it was used
    ZTrustronControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTrustron Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Trustron, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTrustron Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTSN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTSN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTSN "));
    ui->labelzTSNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTSNSyncStatus->setVisible(fShow);
}
